...
import { determineAdaptivePerformanceProfile, obfuscationIntensityForProfile } from './src/services/performance';
...
const defaultRules: ProviderRule[] = [
  {
    ...
    notifyOnBlock: true,
    locationRadiusMiles: 45,
    allowExceptions: ['maps.google.com']
  },
  {
    provider: 'Social',
    locationMasking: true,
...
export default function App() {
  ...
  const compatibility = useMemo(() => resolveCompatibilityDecision(activeDecision), [activeDecision]);
  const effectiveDecision = compatibility.effectiveDecision;
  ...
  const performanceProfile = useMemo(
    () =>
      determineAdaptivePerformanceProfile({
        ...
        batteryImpactHint: runtime.batteryImpactHint
      }),
    [runtime.wakeups, runtime.crashes, runtime.batteryImpactHint]
  );

  const webHeaders = useMemo(
    () => randomizedHeaders(effectiveDecision, runtime.activeSessionId),
    [effectiveDecision, runtime.activeSessionId]
  );
  ...
  useEffect(() => {
    ...
    const metricsInterval = setInterval(async () => {
      ...
      recordMetric('observability_queue_depth', obfuscateMetricValue('observability_queue_depth', observabilityQueueDepth(), effectiveDecision));
    }, 5000);
  ...
  useEffect(() => {
    configureBackgroundPrivacy(
      () => resolveCompatibilityDecision(resolvePrivacyDecision(state, runtime.activeProvider)).effectiveDecision,
      async (summary) => {
        setRuntime((prev) => ({
          ...prev,
  ...
  }, [state, runtime.activeProvider, performanceProfile]);

  const syntheticSearchPreview = useMemo(
    () => mutateSearchQuery('best restaurants near new york', effectiveDecision),
    [effectiveDecision]
  );

  const deviceSignalPreview = useMemo(
    () => obfuscateDeviceSignals(effectiveDecision, runtime.activeSessionId),
    [effectiveDecision, runtime.activeSessionId]
  );

  const interactionDelays = useMemo(
    () => randomizedInteractionDelays(effectiveDecision, obfuscationIntensityForProfile(performanceProfile)),
    [effectiveDecision, performanceProfile]
  );
    ...
    <SafeAreaView style={styles.root}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.logCard}>
          ...
          <Text style={styles.logText}>Adaptive profile: {performanceProfile}</Text>
          <Text style={styles.logText}>Compatibility guard: {compatibility.strictModeApplied ? 'ACTIVE' : 'OFF'}</Text>
          <Text style={styles.logText}>Compatibility notes: {compatibility.notices.join(' | ') || 'none'}</Text>
          <Text style={styles.logText}>Last event: {eventLog}</Text>
          <Text style={styles.logText}>Query preview: {syntheticSearchPreview}</Text>
          <Text style={styles.logText}>Pseudo device: {deviceSignalPreview.pseudoDeviceId}</Text>
          <Text style={styles.logText}>Pseudo ad id: {deviceSignalPreview.pseudoAdId}</Text>
          <Text style={styles.logText}>Network profile: {deviceSignalPreview.networkProfile}</Text>
          <Text style={styles.logText}>TZ bucket: {deviceSignalPreview.timezone}</Text>
          <Text style={styles.logText}>Interaction delays (scroll/click/type): {interactionDelays.scrollDelayMs}/{interactionDelays.clickDelayMs}/{interactionDelays.typingDelayMs}ms</Text>
        ...
        <WebView
          ...
          onShouldStartLoadWithRequest={(request) => {
            const decision = resolveCompatibilityDecision(resolveDecisionForUrl(state, request.url)).effectiveDecision;
          ...
          onMessage={(event) => {
            ...
            if (effectiveDecision.notifyOnBlock) {
