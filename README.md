...
- Cross-app/provider data collection is reduced by applying the same tunnel-side policy controls to all destinations, not just browser traffic.
...
- Added expanded dynamic obfuscation vectors beyond location/query/header only: pseudo device IDs, pseudo ad IDs, network/timezone buckets, interaction-delay randomization, and telemetry metric jittering.


### 8) Compatibility, legal-safety and adaptive performance guards

- Added compatibility/legal guardrails to automatically relax obfuscation for regulated/high-accuracy providers (`Banking`, `Healthcare`, `Government`, `Navigation`, `LocalNetwork`).
- Added adaptive performance profiles (`HIGH_PRIVACY`/`BALANCED`/`LOW_POWER`) to tune background delay ranges and obfuscation intensity for battery/resource constraints.
- Added model-agnostic fallback (`GenericWeb`) so uncataloged services still receive enforceable privacy policy defaults.
...
- Dynamic per-provider policy routing in app runtime with model/service-agnostic fallback (`GenericWeb`).
- Background tasks refresh session/location obfuscation state without manual user intervention.
- Native tunnel is authoritative when active; WebView only supplements detections.
- Android-specific runtime path includes obfuscated device/network headers, query mutation, interaction-delay randomization, and telemetry metric jittering.
