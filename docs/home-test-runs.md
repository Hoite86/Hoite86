...
5. Validate obfuscation surface beyond headers: pseudo device IDs, pseudo ad IDs, network/timezone buckets, interaction delay noise, and metric jitter logs.
6. Validate regulated-provider compatibility guard: for banking/healthcare/government/navigation destinations, location/header/query obfuscation should auto-relax and produce a compatibility notice.
7. **Pass signature:** stage transitions and allow exceptions behave deterministically.
...
## Android-specific, model-agnostic positioning

- Enforcement path is Android-native (VpnService + RN bridge), but policy routing is intentionally model/service agnostic via provider groups and a `GenericWeb` fallback.
- Validate at least one unknown/uncataloged domain in every test run and confirm it maps to `GenericWeb` with the expected default policy behavior.
