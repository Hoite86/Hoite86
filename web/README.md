...
## Fastest move to your Google Cloud project

If you want a clean package you can download and deploy quickly:

1. From repo root, create the archive:
```bash
./scripts/export-renovarix-cmmc.sh
```

2. Download `dist/Renovarix-CMMC.tar.gz`, extract it, and open the app folder:
```bash
tar -xzf Renovarix-CMMC.tar.gz
cd web
```

3. Deploy directly from source (fastest path, Cloud Build handles containerization):
```bash
gcloud auth login
...
4. Set runtime env vars on Cloud Run (if not set during deploy):
```bash
gcloud run services update cmmc-mvp \
  --region us-central1 \
  --update-env-vars NEXT_PUBLIC_FIREBASE_API_KEY=...,NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN=...,NEXT_PUBLIC_FIREBASE_PROJECT_ID=...,NEXT_PUBLIC_FIREBASE_APP_ID=...,FIREBASE_PROJECT_ID=...,FIREBASE_CLIENT_EMAIL=...,FIREBASE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\\n...\\n-----END PRIVATE KEY-----\\n"
```
