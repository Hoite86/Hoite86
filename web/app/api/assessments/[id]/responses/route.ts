...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const level = (assessmentDoc.data()?.level ?? "L2") as AssessmentLevel;
    if (!getControlById(level, parsed.controlId)) {
      return apiError(400, "Invalid controlId for assessment level");
    }
    ...
    const sprsScore = calculateSprs(level, allResponses);
    await assessmentRef.update({
      sprsScore,
      updatedAt: FieldValue.serverTimestamp(),
    });

    invalidateCache(`assessment:${uid}:${parsedParams.id}`);
