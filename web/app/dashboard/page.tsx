    ...
    <main>
      ...
      <section className="mx-auto max-w-4xl space-y-6 p-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/assessment/new" className="rounded bg-blue-600 px-4 py-2 text-white">New Assessment</Link>
        </div>
        ...
        {latest ? (
          <div className="rounded bg-white p-5 shadow">
            ...
            <Link className="mt-3 inline-block text-blue-600 underline" href={`/assessment/${latest.id}`}>Continue</Link>
          </div>
        ) : (
        ...
        <ul className="space-y-2">
          {assessments.map((assessment) => (
            <li key={assessment.id} className="rounded border bg-white p-3">
              <Link href={`/assessment/${assessment.id}`} className="font-medium text-blue-600">{assessment.name}</Link>
